<template>
    <div class="head">
        <div class="btn-box">
            <el-button v-if="noUser" @click="dialogFormLoginVisible = true">登录</el-button>
            <el-button v-if="noUser" el-icon-switch-button @click="dialogFormRegisterVisible = true">注册</el-button>
            <el-button v-if="!noUser" @click="exit">退出</el-button>
        </div>
        <el-dialog title="登录" :visible.sync="dialogFormLoginVisible" width="10rem">
            <el-form :model="formLogin" status-icon :rules="rules" ref="formLogin">
                ...
                <el-input v-model="formLogin.username" autocomplete="off" placeholder="请输入账号" style="width: 5rem"></el-input>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button type="text" style="float: right;font-size:0.3rem;color:#000" @click="goRegister">还没有账号?点击去注册</el-button>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="注册" :visible.sync="dialogFormRegisterVisible" width="10rem">
            <el-form :model="formRegister" status-icon :rules="rules" ref="formRegister">
                ...
                <el-form-item>
                    <el-button type="text" style="float: right;font-size:0.3rem;color:#000" @click="goLogin">已有账号?点击去登录</el-button>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                    ...
                    this.$refs[formName].validate((valid) => {
                        if (valid) {
                            if(formName === 'formLogin'){
                                ...
                                //发送Ajax验证用户登录信息
                                axios.post(`http://localhost:8080/userLogin/api/users/login/`,{
                                    //请求体信息，发送用户登录信息
                                    ...
                                    if(response.data.code >= 200 && response.data.code < 300){
                                        this.$bus.$emit('noUser', false);
                                        this.noUser = false;
                                        alert('登录成功')
                                        window.location.href='http://localhost:8080/#/activitySquare'
                                    }else{
                                        //状态码不为2xx，登陆失败
                                        alert('昵称或密码错误')
                                    }
